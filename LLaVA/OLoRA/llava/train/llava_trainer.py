...
class LLaVATrainer(Trainer):
    ...
    def training_step(self, model, inputs, num_items_in_batch=None) -> torch.Tensor:
        ...
        do_grad_scaling = hasattr(self, "scaler") and self.scaler is not None
        ...
        if labels is not None:
            ...
            if self.cur_task >= 1:
                for name, param in model.named_parameters():
                    if f"default.loraA.{self.cur_task}.weight" in name:
                        current_loraA = param
                        for prev_task in range(self.cur_task):
                            for name_, param_ in model.named_parameters():
                                if f"default.loraA.{prev_task}.weight" in name_ and name.split("loraA")[0] == name_.split("loraA")[0]:
                                    orthogonal_loss += torch.abs(torch.mm(current_loraA, param_.T)).sum()
                                    break  
